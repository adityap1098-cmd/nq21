...
import { getReportJasa, type JasaDetailLine, type MekanikJasaStats } from '@/store/selectors'
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8 }}>
        <div>
          <div style={{ fontFamily: 'var(--display)', fontSize: 22, lineHeight: 1.1 }}>
            {stat.jobsCount}
          </div>
          <div style={{ fontSize: 9, fontFamily: 'var(--mono)', color: 'var(--text-muted)', letterSpacing: '0.08em', marginTop: 2 }}>
            JASA
          </div>
        </div>
        <div>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 600 }}>
            {fmtRp(stat.totalBasis)}
          </div>
          <div style={{ fontSize: 9, fontFamily: 'var(--mono)', color: 'var(--text-muted)', letterSpacing: '0.08em', marginTop: 2 }}>
            BASIS
          </div>
        ...
        <div>
          <div style={{ fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 700, color: 'var(--accent)' }}>
            {fmtRp(stat.totalKomisi)}
          </div>
          <div style={{ fontSize: 9, fontFamily: 'var(--mono)', color: 'var(--text-muted)', letterSpacing: '0.08em', marginTop: 2 }}>
            KOMISI
          </div>
    ...
    <div>
      {line.mechanics.map((m, i) => (
        ...
        >
          ...
          {/* Rate — with strikethrough if override */}
          <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--text-muted)' }}>
...
export default function LaporanJasaPage() {
  ...
  const jasaCategories = useMemo(
    () => categories.filter((c) => c.isJasa && c.isActive),
    [categories],
  )
  ...
  // Unfiltered report for KPI cards (period only, no mechanic/category filter)
  const allReport = useMemo(
    () => getReportJasa(transactions, lines, lineMechanics, rates, categoryMap, customers, mechanics, range),
    [transactions, lines, lineMechanics, rates, categoryMap, customers, mechanics, range],
  )
  ...
  const filteredReport = useMemo(
    ...
    [transactions, lines, lineMechanics, rates, categoryMap, customers, mechanics, range, mechanicFilter, categoryFilter, allReport],
  )

  // All active + inactive mechanics who appear in allReport, plus any with 0 jobs
  const kpiMechanics = useMemo(() => {
    ...
    return mechanics.map((m) => {
      ...
      if (stat) return stat
      return { mechanicId: m.id, mechanicName: m.name, isActive: m.isActive, jobsCount: 0, totalBasis: 0, totalKomisi: 0, uniqueCategories: 0 }
    })
  ...
  function emptyMsg(): string {
    ...
    if (categoryFilter.length > 0 && mechanicFilter.length === 0) {
      return 'Tidak ada jasa di kategori yang dipilih.'
    }
    ...
    <div style={{ padding: '28px 32px', maxWidth: 1280 }}>
        ...
        action={
          <button
            ...
            style={{
              ...
              borderRadius: 6,
              fontSize: 12, fontFamily: 'var(--mono)', fontWeight: 700, letterSpacing: '0.08em',
              cursor: 'pointer',
            }}
          >
            ↓ EXPORT CSV
      ...
      {kpiMechanics.length > 0 && (
        <div
          style={{
            display: 'grid',
            gridTemplateColumns: `repeat(${Math.min(kpiMechanics.length, 4)}, 1fr)`,
            gap: 12,
            marginBottom: 16,
          }}
        >
          {kpiMechanics.map((stat) => (
            <MechanicKpiCard
              ...
              isInactive={!mechanicMap[stat.mechanicId]?.isActive}
          ...
          <div style={{ display: 'inline-flex', background: 'var(--surface-alt)', borderRadius: 6, padding: 3, gap: 2 }}>
            <button
              ...
              style={{
                ...
                background: mechanicFilter.length === 0 ? 'var(--text)' : 'transparent',
                color: mechanicFilter.length === 0 ? '#fff' : 'var(--text-muted)',
                cursor: 'pointer',
              }}
            >
              Semua
              ...
              return (
                ...
                >
                  <div style={{
                    ...
                    fontSize: 8, fontFamily: 'var(--display)', color: active ? '#fff' : 'var(--text-muted)',
                  }}>
                    {stat.mechanicName[0]}
                  </div>
                  {stat.mechanicName}
                  <span style={{ background: active ? 'rgba(255,255,255,0.2)' : 'var(--surface)', padding: '0 4px', borderRadius: 3, fontSize: 9 }}>
                    {stat.jobsCount}
                  </span>
                </button>
              )
          ...
          <div style={{ display: 'inline-flex', background: 'var(--surface-alt)', borderRadius: 6, padding: 3, gap: 2 }}>
            <button
              ...
              style={{
                fontFamily: 'var(--mono)', fontSize: 10, fontWeight: 600, letterSpacing: '0.06em',
                padding: '5px 10px', borderRadius: 4, border: 'none',
                background: categoryFilter.length === 0 ? 'var(--text)' : 'transparent',
                color: categoryFilter.length === 0 ? '#fff' : 'var(--text-muted)',
                cursor: 'pointer',
              }}
            >
              Semua
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginBottom: 16 }}>
        {[
          ...
          >
            <div style={{ fontSize: 9, fontFamily: 'var(--mono)', letterSpacing: '0.12em', color: 'var(--text-muted)', marginBottom: 6 }}>
              {label}
            </div>
            <div style={{
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 20px', borderBottom: '1px solid var(--border)' }}>
          ...
          <span style={{
            ...
            background: 'var(--surface-alt)', fontSize: 10, fontFamily: 'var(--mono)',
            color: 'var(--text-muted)', fontWeight: 700, letterSpacing: '0.08em',
          }}>
        ...
        {detailLines.length === 0 ? (
          <div style={{ padding: '60px 24px', textAlign: 'center', color: 'var(--text-muted)' }}>
            ...
            {hasFilters ? (
              <button
                onClick={resetFilters}
                style={{ padding: '8px 16px', border: '1px solid var(--border)', borderRadius: 6, background: 'var(--surface)', cursor: 'pointer', fontSize: 12, fontFamily: 'var(--mono)' }}
              >
                ↺ Reset Filter
              </button>
            ) : (
              <button
                ...
                style={{ padding: '8px 16px', border: 'none', borderRadius: 6, background: 'var(--accent)', color: '#fff', cursor: 'pointer', fontSize: 12, fontFamily: 'var(--mono)', fontWeight: 700 }}
              >
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13, minWidth: 1000 }}>
                ...
                <tr style={{ borderBottom: '1px solid var(--border)' }}>
                  {['TGL · REF', 'CUSTOMER', 'KATEGORI', 'NOMINAL', 'MATERIAL', 'BASIS', 'MEKANIK · SHARE · KOMISI'].map((h) => (
                    <th
                      key={h}
                      style={{
                        padding: h === 'TGL · REF' ? '10px 12px 10px 20px' : '10px 12px',
                        textAlign: ['NOMINAL','MATERIAL','BASIS'].includes(h) ? 'right' : 'left',
                ...
                {detailLines.map((line) => (
                  ...
                  >
                    ...
                    <td style={{ padding: '10px 12px', maxWidth: 160 }}>
                      ...
                      {line.customerMotor && (
                        <div style={{ fontSize: 10, color: 'var(--text-muted)', marginTop: 1 }}>{line.customerMotor}</div>
                      )}
                    </td>
                    {/* KATEGORI */}
                    <td style={{ padding: '10px 12px' }}>
                      <span style={{
                        ...
                        background: 'var(--surface-alt)', fontSize: 11, fontFamily: 'var(--mono)', fontWeight: 600,
                      }}>
                        {line.categoryName}
                      </span>
                      {line.itemName && (
                        <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>{line.itemName}</div>
                      )}
                    </td>
                    {/* NOMINAL */}
                    <td style={{ padding: '10px 12px', textAlign: 'right', fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap' }}>
                      {fmtRp(line.nominal)}
                    </td>
                    {/* MATERIAL */}
                    ...
                    {/* BASIS */}
                    <td style={{ padding: '10px 12px', textAlign: 'right', fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, whiteSpace: 'nowrap' }}>
                      {fmtRp(line.basis)}
                    </td>
                    {/* MEKANIK stack */}
                    <td style={{ padding: '10px 12px' }}>
              ...
              <tfoot>
                <tr style={{ background: 'var(--bg)', borderTop: '2px solid var(--border)' }}>
                  <td style={{ padding: '10px 12px 10px 20px', fontFamily: 'var(--mono)', fontSize: 11, letterSpacing: '0.08em', fontWeight: 700 }} colSpan={3}>
                    GRAND TOTAL · {detailLines.length} LINE
                  </td>
                  <td style={{ padding: '10px 12px', textAlign: 'right', fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700 }}>
                    {fmtRp(detailLines.reduce((s, l) => s + l.nominal, 0))}
                  </td>
                  <td style={{ padding: '10px 12px', textAlign: 'right', fontFamily: 'var(--mono)', fontSize: 12, color: 'var(--text-muted)' }}>
                    {fmtRp(detailLines.reduce((s, l) => s + l.biayaMaterial, 0))}
                  </td>
                  <td style={{ padding: '10px 12px', textAlign: 'right', fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700 }}>
                    {fmtRp(summary.totalBasis)}
                  </td>
                  <td style={{ padding: '10px 12px', fontFamily: 'var(--mono)', fontSize: 13, fontWeight: 700, color: 'var(--accent)' }}>
                    {fmtRp(summary.totalKomisi)} <span style={{ fontSize: 9, color: 'var(--text-muted)', marginLeft: 4 }}>KOMISI</span>
                  </td>
                </tr>
              </tfoot>
            </table>
