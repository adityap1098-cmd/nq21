...
export default function LaporanCashFlowPage() {
  ...
  const filteredTx = useMemo(() => {
    let result = txsWithLines
    if (tipeFilter !== 'all') result = result.filter(t => t.tipe === tipeFilter)
    if (metodeFilter !== 'all') result = result.filter(t => t.payment_method === metodeFilter)
    return result.sort((a, b) => {
      const dc = a.tgl.localeCompare(b.tgl)
      return dc !== 0 ? dc : a.created_at.localeCompare(b.created_at)
    })
  }, [txsWithLines, tipeFilter, metodeFilter])

  const tableData = useMemo<TableRow[]>(() => {
    let saldo = 0
    return filteredTx.map(t => {
      const delta = t.tipe === 'income' ? t.total_nominal : -t.total_nominal
      saldo += delta
      const catNames = t.transaction_lines
        .map(l => l.categories?.name)
        .filter((n): n is string => !!n)
      const unique = catNames.filter((n, i, arr) => arr.indexOf(n) === i)
      return {
        id: t.id,
        ...
        tipe: t.tipe,
        partyName: t.tipe === 'income'
          ? t.customer?.name ?? '—'
          : t.supplier?.name ?? '—',
        paymentMethod: t.payment_method,
        totalNominal: t.total_nominal,
        delta,
        saldoRunning: saldo,
  ...
  const summary = useMemo(() => {
    const totalMasuk   = filteredTx.filter(t => t.tipe === 'income' ).reduce((s, t) => s + t.total_nominal, 0)
    const totalKeluar  = filteredTx.filter(t => t.tipe === 'expense').reduce((s, t) => s + t.total_nominal, 0)
    const saldoBersih  = tableData.length > 0 ? tableData[tableData.length - 1].saldoRunning : 0
    return {
      totalMasuk, totalKeluar, saldoBersih,
      countMasuk:  filteredTx.filter(t => t.tipe === 'income' ).length,
      countKeluar: filteredTx.filter(t => t.tipe === 'expense').length,
    }
  ...
  return (
    <div style={{ padding: '28px 32px', maxWidth: 1280 }}>
        ...
        action={
          <button
            ...
            style={{
              ...
              fontSize: 12, fontFamily: 'var(--mono)', fontWeight: 700, letterSpacing: '0.08em', cursor: 'pointer',
            }}
          >
            ↓ EXPORT CSV
          </button>
      ...
      <div style={{
        ...
        <PeriodSelector value={preset} onChange={(v) => { setPreset(v as typeof preset); resetPage() }} />
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 16, marginBottom: 20 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <FilterPillGroup options={TIPE_OPTIONS} value={tipeFilter} onChange={(v) => { setTipeFilter(v as TipeFilter); resetPage() }} />
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--text-muted)', letterSpacing: '0.1em' }}>METODE</span>
          <FilterPillGroup options={METODE_OPTIONS} value={metodeFilter} onChange={(v) => { setMetodeFilter(v as MetodeFilter); resetPage() }} />
        </div>
        {hasFilters && (
          <button
            onClick={() => { setTipeFilter('all'); setMetodeFilter('all'); resetPage() }}
            style={{
              marginLeft: 'auto', fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--text-muted)',
              background: 'none', border: '1px solid var(--border)', borderRadius: 5, padding: '4px 10px', cursor: 'pointer',
            }}
          >
            ↺ Reset Filter
          </button>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 20 }}>
        <div style={{ background: 'rgba(22,163,74,0.06)', border: '1px solid rgba(22,163,74,0.2)', borderRadius: 8, padding: '14px 18px' }}>
          <div style={{ fontSize: 10, fontFamily: 'var(--mono)', letterSpacing: '0.12em', color: '#16a34a', marginBottom: 6 }}>TOTAL MASUK</div>
          <div style={{ fontFamily: 'var(--display)', fontSize: 24, color: '#16a34a' }}>{fmtRp(summary.totalMasuk)}</div>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>{summary.countMasuk} transaksi</div>
        </div>
        <div style={{ background: 'rgba(220,38,38,0.06)', border: '1px solid rgba(220,38,38,0.2)', borderRadius: 8, padding: '14px 18px' }}>
          <div style={{ fontSize: 10, fontFamily: 'var(--mono)', letterSpacing: '0.12em', color: 'var(--accent)', marginBottom: 6 }}>TOTAL KELUAR</div>
          <div style={{ fontFamily: 'var(--display)', fontSize: 24, color: 'var(--accent)' }}>{fmtRp(summary.totalKeluar)}</div>
          <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 4 }}>{summary.countKeluar} transaksi</div>
        </div>
        <div style={{
          ...
          <div style={{ fontSize: 10, fontFamily: 'var(--mono)', letterSpacing: '0.12em', color: saldoPositif ? 'rgba(255,255,255,0.55)' : 'var(--accent)', marginBottom: 6 }}>SALDO BERSIH</div>
      ...
      <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 20px', borderBottom: '1px solid var(--border)' }}>
          <span style={{ fontFamily: 'var(--display)', fontSize: 14, letterSpacing: '0.04em' }}>RINCIAN KRONOLOGIS</span>
          <span style={{ padding: '2px 8px', borderRadius: 4, background: 'var(--surface-alt)', fontSize: 10, fontFamily: 'var(--mono)', color: 'var(--text-muted)', fontWeight: 700, letterSpacing: '0.08em' }}>
            {isLoading ? '...' : `${tableData.length} ENTRI`}
          </span>
        </div>

        {isLoading ? (
          <div style={{ padding: '60px 24px', textAlign: 'center', color: 'var(--text-muted)', fontFamily: 'var(--mono)', fontSize: 12 }}>Memuat...</div>
        ) : tableData.length === 0 ? (
          <div style={{ padding: '60px 24px', textAlign: 'center', color: 'var(--text-muted)' }}>
            ...
            <div style={{ fontSize: 13, marginBottom: 16 }}>
              {hasFilters ? 'Tidak ada transaksi yang cocok dengan filter.' : 'Belum ada cash flow di periode ini.'}
            </div>
            {hasFilters ? (
              <button onClick={() => { setTipeFilter('all'); setMetodeFilter('all') }} style={{ padding: '8px 16px', border: '1px solid var(--border)', borderRadius: 6, background: 'var(--surface)', cursor: 'pointer', fontSize: 12, fontFamily: 'var(--mono)' }}>↺ Reset Filter</button>
            ) : (
              <button onClick={() => navigate('/transaksi/baru')} style={{ padding: '8px 16px', border: 'none', borderRadius: 6, background: 'var(--accent)', color: '#fff', cursor: 'pointer', fontSize: 12, fontFamily: 'var(--mono)', fontWeight: 700 }}>+ Input Transaksi</button>
            )}
          </div>
        ) : (
          <>
                  ...
                  <tr style={{ borderBottom: '1px solid var(--border)' }}>
                    {(['TGL', 'JAM', 'NO REFERENSI', 'PARTY', 'KATEGORI', 'METODE', 'NOMINAL', 'SALDO'] as const).map(h => (
                      <th key={h} style={{
                        ...
                        fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: '0.1em', color: 'var(--text-muted)', fontWeight: 600, whiteSpace: 'nowrap',
                      }}>
                        {h}
                      </th>
                    ))}
                  ...
                  {pagedData.map(row => {
                    ...
                    return (
                      <tr
                        ...
                        style={{ borderBottom: '1px solid var(--border)', borderLeft: `3px solid ${isIncome ? '#16a34a' : 'var(--accent)'}`, cursor: 'pointer', transition: 'background 0.1s' }}
                        onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = 'var(--bg)' }}
                        onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = '' }}
                      >
                        <td style={{ padding: '11px 12px 11px 20px', whiteSpace: 'nowrap' }}>
                          <div style={{ fontWeight: 600, fontSize: 12.5 }}>{fmtShortDate(row.tgl)}</div>
                        </td>
                        <td style={{ padding: '11px 12px', whiteSpace: 'nowrap' }}>
                          <div style={{ fontFamily: 'var(--mono)', fontSize: 11, color: 'var(--text-muted)' }}>{fmtTime(row.createdAt)}</div>
                        </td>
                        <td style={{ padding: '11px 12px', whiteSpace: 'nowrap' }}>
                          <span style={{ fontFamily: 'var(--mono)', fontSize: 11, fontWeight: 700, color: 'var(--text)' }}>{row.noReferensi}</span>
                        </td>
                        <td style={{ padding: '11px 12px', maxWidth: 160 }}>
                          <div style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontWeight: 500, fontSize: 12.5 }}>{row.partyName}</div>
                        </td>
                        <td style={{ padding: '11px 12px' }}>
                          {row.categoryNames.length === 0 ? (
                            ...
                            <span style={{ fontSize: 12, color: 'var(--text-secondary, var(--text-muted))' }}>
                              {row.categoryNames[0]}{row.categoryNames.length > 1 && (
                                <span style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--text-muted)', marginLeft: 4 }}>+{row.categoryNames.length - 1}</span>
                              )}
                            </span>
                          )}
                        </td>
                        <td style={{ padding: '11px 12px', whiteSpace: 'nowrap' }}><MethodBadge method={row.paymentMethod} /></td>
                        <td style={{ padding: '11px 12px', textAlign: 'right', whiteSpace: 'nowrap' }}>
                          <span style={{ fontFamily: 'var(--mono)', fontSize: 12, fontWeight: 700, color: isIncome ? '#16a34a' : 'var(--accent)' }}>
                            {isIncome ? '+' : '−'} {fmtRp(row.totalNominal)}
                          </span>
                        </td>
                        <td style={{ padding: '11px 20px 11px 12px', textAlign: 'right', whiteSpace: 'nowrap' }}>
                          <span style={{ fontFamily: 'var(--mono)', fontSize: 12, fontWeight: saldoNeg ? 700 : 500, color: saldoNeg ? 'var(--accent)' : 'var(--text)' }}>
            ...
            {totalPages > 1 && (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px', borderTop: '1px solid var(--border)', background: 'var(--bg)' }}>
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <button disabled={page === 1} onClick={() => setPage(p => p - 1)} style={{ padding: '5px 12px', border: '1px solid var(--border)', borderRadius: 5, background: 'var(--surface)', cursor: page === 1 ? 'not-allowed' : 'pointer', opacity: page === 1 ? 0.4 : 1, fontSize: 12, fontFamily: 'var(--mono)' }}>← Prev</button>
                  <button disabled={page === totalPages} onClick={() => setPage(p => p + 1)} style={{ padding: '5px 12px', border: '1px solid var(--border)', borderRadius: 5, background: 'var(--surface)', cursor: page === totalPages ? 'not-allowed' : 'pointer', opacity: page === totalPages ? 0.4 : 1, fontSize: 12, fontFamily: 'var(--mono)' }}>Next →</button>
                </div>
              </div>
            )}
          </>
      ...
      {!saldoPositif && tableData.length > 0 && (
        <div style={{ marginTop: 12, padding: '10px 16px', background: 'rgba(220,38,38,0.06)', border: '1px solid rgba(220,38,38,0.2)', borderRadius: 6, fontSize: 12, color: 'var(--accent)', display: 'flex', alignItems: 'center', gap: 8 }}>
          <span>⚠</span>
