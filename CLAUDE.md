...
- **M006-V2 Post-tag — date format centralized (2026-05-14)**: `src/lib/format.ts` → `formatWeekRange(weekStart, weekEnd)` outputs "11–17 Mei 2026". Safe with both `"YYYY-MM-DD"` and full ISO timestamp inputs (uses `.slice(8,10)` + `parseInt`). Replaces ad-hoc `.slice(8)` usage in Dashboard.tsx which broke on ISO timestamps from Supabase.
- **M006-V2 Post-tag — ISO timestamp normalization at hook (2026-05-14)**: `useCommissionPeriods` maps `weekStart/weekEnd` via `.slice(0,10)`. Supabase returns `date` columns as `"YYYY-MM-DD"` in most cases, but schema may store as `timestamptz` → returns `"2026-05-11T00:00:00+00:00"`. Normalizing at the hook protects ALL consumers (Dashboard chart builder, Topbar parseLocalDate, komisi util formatters) without per-callsite patches.
- **M006-V2 Post-tag — chart tickFormatter NaN guard (2026-05-14)**: `fmtRpShort` in Dashboard.tsx prefixed with `if (isNaN(n)) return '0'`. Defensive against chart data pipeline producing NaN values (e.g., ISO timestamp leak into addDaysToStr → NaN date buckets). Belt-and-suspenders — root cause fixed by hook normalization, guard protects against future regressions.
